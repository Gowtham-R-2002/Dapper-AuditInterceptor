...
public class EntitySnapshotAuditableDbCommand : DbCommand
{
    ...
    private async Task<object> ExecuteScalarWithEntitySnapshotAuditAsync()
    {
        ...
        try
        {
            ...
            await LogAuditEventWithSnapshotAsync(commandText, parameters, entitySnapshot, sqlStatement);
    ...
    private async Task LogAuditEventWithSnapshotAsync(string commandText, Dictionary<string, object> parameters, Dictionary<string, object> beforeSnapshot, SqlStatement sqlStatement)
    {
        try
        {
            var auditContext = _auditContextProvider?.GetCurrentContext();

            // Calculate after image based on operation type and parameters
            var afterSnapshot = CalculateAfterImage(beforeSnapshot, sqlStatement, parameters);
